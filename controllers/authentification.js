...
module.exports.signInEleve = async (request, response) => {
    let { nom, postnom, prenom, dateNaiss, age, adresse, telephone, psw} = request.body;
    ...
    try {
        (await EleveModel.findOne({telephone: telephone}).then(async(data) => {
            if (data){
                return response.status(400).json({ error: 'Ce numéro de téléphone est déjà utilisé, Veuillez utiliser un autre' });
            }
            else{
                (await EleveModel.create({
                    ...
                    postnom: postnom,
                    prenom: prenom,
                    dateNaissaince: dateNaiss,
                    age: age,
                    adresseLocale: adresse,
                    telephone: telephone,
                    password: password,
                    idenfient: idEleve
                    })).save().then((data) => {
                        if(data) {
                            ...
                            response.cookie('jwt', token, { httpOnly: true, maxAge: 365 * 24 * 60 * 60 * 1000 });
...
module.exports.loginEleve = async(request, response) => {
    ...
    try {
        (await EleveModel.findOne({$or: [{telephone: login}, {idenfient: login}]}).then((data) => {
            if(data){
                (async () => {
                ...
                return response.status(400).send('Numéro de téléphone ou identifiant incorrect');
            }
        }))
    } catch (error) {
        console.error(error);
...
module.exports.signInComptable = async (request, response) => {
    ...
    try {
        (await ComptableModel.findOne({telephone: telephone}).then(async(data) => {
            if (data){
                return response.status(400).json({ error: 'Ce numéro de téléphone est déjà utilisé, Veuillez utiliser un autre' });
            }
            else{
...
module.exports.loginComptable = async(request, response) => {
    ...
    try {
        (await ComptableModel.findOne({$or: [{telephone: login}, {matricule: login}]}).then((data) => {
            if(data){
                (async () => {
                    var password = await bcrypt.compareSync(psw, data.password);
                    if(password == true){
